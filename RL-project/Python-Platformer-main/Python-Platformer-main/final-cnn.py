...
def handle_vertical_collision(player, objects, dy, flag):
    collided_objects = []
    for obj in objects:
            ...
            if dy > 0:
                if obj.name == "flying_box":
                    if player.rect.bottom >= HEIGHT - 100:
                        player.hit_head()
                        continue
                elif obj.name == "green_enemy":
                    # if (player.rect.bottom <= obj.rect.top
                    #     and obj.rect.x <= player.rect.x  and player.rect.x <= obj.rect.x + obj.rect.width):
                        print("debug")
                        # obj.make_hit()
                        obj.destroy() 
                        print("khjkffv")
...
def collide(player, objects, dx):
    ...
    for obj in objects:
        if (pygame.sprite.collide_mask(player, obj)
            and player.rect.bottom >= HEIGHT - 96):
...
def handle_move(player, objects, flag):
    ...
    vertical_collide = handle_vertical_collision(player, objects, player.y_vel, flag)
...
def movement(player, objects, action):
    ...
    vertical_collide = handle_vertical_collision(player, objects, player.y_vel, False)
    to_check = [*vertical_collide]
...
def main(window):
    ...
    player = Player(block_size * 30,HEIGHT - block_size - 100,50,50)

    fire1 = Fire(block_size * 19.5, HEIGHT - block_size * 1 - 64, 16, 32)
    fire2 = Fire(block_size * 34.25, HEIGHT - block_size * 2 - 64, 16, 32)
    fire3 = Fire(block_size * 44.5, HEIGHT - block_size * 1 - 64, 16, 32)

    flag = Checkpoint(block_size * 60, HEIGHT - block_size - 128)

    fire1.on()
    fire2.on()
    fire3.on()

    attacking_block1 = Flying_enemy(block_size * 10, HEIGHT - block_size * 3.5 - enemy_size, enemy_size, enemy_size)
    attacking_block2 = Flying_enemy(block_size * 24, HEIGHT - block_size * 2 - enemy_size, enemy_size, enemy_size)

    green_enemy1 = Green_enemy(block_size * 17, HEIGHT - block_size - green_enemy_size - spike_size, green_enemy_size, enemy_size)
    green_enemy2 = Green_enemy(block_size * 42, HEIGHT - block_size - green_enemy_size - spike_size, green_enemy_size, enemy_size)

    g_enemy = [green_enemy1, green_enemy2]

    fire1.on()
    fire2.on()
    fire3.on()

    attacking_block1.fly()
    attacking_block2.fly()

    green_enemy1.movement(player)
    green_enemy2.movement(player)

    floor = [Block(i * block_size, HEIGHT - block_size, block_size)
             for i in range(0, (WIDTH * 6) // block_size)]
    objects = [*floor, 
        ...
        #    Block(block_size * 15, HEIGHT - block_size * 3, block_size),
           ...
           # for the steps block
           Block(block_size * 51, HEIGHT - block_size * 2, block_size),  
           Block(block_size * 52, HEIGHT - block_size * 2, block_size),
           Block(block_size * 52, HEIGHT - block_size * 3, block_size), 
           Block(block_size * 53, HEIGHT - block_size * 2, block_size),  
           Block(block_size * 53, HEIGHT - block_size * 3, block_size),  
        ...
        #    Spikes(block_size * 5, HEIGHT - block_size - spike_size * 2, spike_size, spike_size),
           ...
           Spikes(block_size * 28, HEIGHT - block_size - spike_size * 2, spike_size, spike_size),
           Spikes(block_size * 28 + spike_size * 2, HEIGHT - block_size - spike_size * 2, spike_size, spike_size),
           Spikes(block_size * 28 + spike_size * 4, HEIGHT - block_size - spike_size * 2, spike_size, spike_size), 
    ...
    while run:
        ...
        flag.loop()
        attacking_block1.loop()
        attacking_block2.loop()
        for ene in g_enemy:
            if ene.alive:
                ene.loop(FPS)
                ene.movement(player)
        ...
        handle_move(player, objects, flags)
        i += 1
...
class Pls_learn(gym.Env):
    def __init__(self):
        ...
        self.g_enemy = [self.green_enemy1, self.green_enemy2]
    ...
    def step(self, action):
        ...
        if self._agent.rect.x >= self._flag.rect.x + 100:
            reward += 100
            self.terminated = True
            return obs, reward, self.terminated, self.truncated, {}
...
def train(env, steps):
    ...
    model = PPO("CnnPolicy", env, learning_rate=0.0002, n_steps=1024, tensorboard_log="platformer_board_cnn", verbose=1)

    # eval_callback = EvalCallback(env, best_model_save_path='logs/', log_path='logs/', eval_freq=1000, deterministic=True, render=False)

    # check_env(env)

    # model = PPO.load("50k-50kpos-game-13.zip", env)

    model.learn(total_timesteps = steps)
    model.save("50k-50k-50kpos-game-14")


def test(env):
    # env = DummyVecEnv([lambda: Pls_learn()])  
    # model = PPO("CnnPolicy", env, verbose=1) 
    model = PPO.load("50k-50k-50kpos-game-14", env=env)
    obs, info = env.reset()
    total_reward = 0
    for _ in range(20):
        terminated = False
