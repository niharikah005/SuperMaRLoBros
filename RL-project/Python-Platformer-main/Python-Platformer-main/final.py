...
def main(window):
    ...
    player = Player(block_size * 20,HEIGHT - block_size - 100,50,50)
...
class Pls_learn(gym.Env):
    def __init__(self):
        ...
        self._spike_size = 16
        self._agent = Player(self._block_size * 20 + 20 ,HEIGHT - self._block_size * 3 - 100,50,50)
        self._x_offset = self._block_size * 18
        self._steps = 0
    ...
    def step(self, action):
        ...
        if pygame.sprite.collide_mask(self._agent, self._flag):
            reward += 100
            self.terminated = True
            return obs, reward, self.terminated, self.truncated, {}
...
def train(env, steps):
    ...
    model = PPO.load("50k-game-11.zip", env)

    model.learn(total_timesteps = steps)
    model.save("50k-50kpos-game-12")


def test(env):
    # env = DummyVecEnv([lambda: Pls_learn()])  
    # model = PPO("MlpPolicy", env, verbose=1) 
    model = PPO.load("50k-game-11", env=env)
    obs, info = env.reset()
    total_reward = 0
    for _ in range(10):
        terminated = False
